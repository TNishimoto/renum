    ...
    namespace lcp_on_rlbwt
    {
        ...
        */
        ...
        public:
            ...
            void move_push(std::array<INDEX_SIZE, CARRAYSIZE> &_childs_vec, std::array<bool, ARRAYSIZE> &_first_child_flag_vec, std::array<bool, ARRAYSIZE> &_maximal_repeat_check_vec,
                           uint64_t &_st_node_count, uint64_t &_child_count)
            {
                uint64_t p = UINT64_MAX;
                for (int64_t i = this->first_child_flag_vec.size() - 1; i >= 0; i--)
                {
                ...
                uint64_t width = fsize - p;
                assert(_child_count + width <= _first_child_flag_vec.size());
                for (uint64_t i = p; i < fsize; i++)
                {
                    _childs_vec[(2 * _child_count)] = (this->childs_vec[(i * 2)]);
                    _childs_vec[(2 * _child_count) + 1] = (this->childs_vec[(i * 2) + 1]);
                    _first_child_flag_vec[_child_count] = this->first_child_flag_vec[i];
                    _child_count++;
                }
                ...
                _maximal_repeat_check_vec[_st_node_count] = this->maximal_repeat_check_vec[this->maximal_repeat_check_vec.size() - 1];
